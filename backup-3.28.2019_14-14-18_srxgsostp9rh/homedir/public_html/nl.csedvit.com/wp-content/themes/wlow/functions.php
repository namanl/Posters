...
/* ------------------------------------------------------------------------- */
	...
	require_once get_template_directory() . '/functions/wlow_bootstrap_navwalker.php';   	// script for bootstrap menu
...
function wlow_top_bar_menu_cb() {
	if ( current_user_can( 'manage_options' ) ) {
		echo '<ul><li><a href="'. esc_url(admin_url( 'nav-menus.php' )) .'">'. esc_html__('Add a Menu','wlow') .'</a></li></ul>';
	}
};
...
if ( ! function_exists( 'wlow_scripts' ) ) {
	...
	function wlow_scripts() {
		...
		$array = array(
			"path_wp" => esc_url_raw(home_url()).'/',
		);
	  wp_localize_script( "wlow-script", "wlow_php_vars", $array );
...
if ( ! function_exists( 'wlow_styles' ) ) {
	...
	function wlow_styles() {
		...
		if ( ! function_exists( 'wlowpro_enqueue_styles' ) ) {
			...
			wp_enqueue_style( 'wlow-montserrat','//fonts.googleapis.com/css?family=Montserrat:300,400,700,800');

		}

		wp_enqueue_style( 'wlow', get_stylesheet_uri() );
