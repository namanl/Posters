Hustle.define("Slidein.View", function($, doc, win){
		"use strict";

		return Hustle.View.extend({
				el: '.wpmudev-hustle-slidein-wizard-view',
				preview: false,
				preview_model: false,
				events: {
	          ...
	          // unset listeners
						...
						$(document).on( 'click', 'ul.wpmudev-feature-image-location li', $.proxy( this.toggle_feature_image_options, this ) );
				...
				apply_custom_css: function() {
					...
					if (customize_css === 1 || customize_css === '1') {
						...
						$.ajax({
								...
								dataType: "json",
								data: {
										action: 'hustle_slidein_prepare_custom_css',
										css: custom_css,
										_ajax_nonce: nonce
								},
				...
				save: function($btn) {
						...
						return $.ajax({
								...
								type: 'POST',
								data: {
										action: 'hustle_save_slidein_module',
										_ajax_nonce: nonce,
										id: id,
										module: module,
										content: content,
				...
				toggle_feature_image_options: function(e) {
						...
						this.content_view.model.set( 'feature_image_location', $input.val(), {silent:true} );
				},
				...
				settings_view_changed: function(model) {
						...
						if ( 'on_exit_intent_delayed' in changed ) {
							...
							if ( $target.length ) {
								if ( changed['on_exit_intent_delayed'] ) {
									$target.addClass('wpmudev-show');
									$target.removeClass('wpmudev-hidden');
								} else {
									$target.removeClass('wpmudev-show');
				...
				after_preview_render: function() {
					var me = this,
							$preview = this.$('#wph-preview-modal')
								// Remove old display position classes.
								.removeClass('inc_opt_slidein_nw inc_opt_slidein_n inc_opt_slidein_ne inc_opt_slidein_w inc_opt_slidein_e inc_opt_slidein_sw inc_opt_slidein_s inc_opt_slidein_se')
								// Add display position class
								.addClass('wpmudev-modal-active inc_opt_slidein_' + this.settings_view.model.get('display_position')),
							$modal = $preview.find('.hustle-modal'),
